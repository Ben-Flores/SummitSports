...
namespace SummitSportsApp
{
    internal class clsSQL
    {
        ...
        public static void CreateNewUser(NewUser n, int position, frmRegister form)
        {
            try
            {
                ...
                cmd.Append("Insert Into " + SCHEMA_NAME + "Logon (PersonID, LogonName, Password, FirstChallengeQuestion, FirstChallengeAnswer, SecondChallengeQuestion, SecondChallengeAnswer, ThirdChallengeQuestion, ThirdChallengeAnswer, PositionID) Values (" +
                    personID + ", '" + n.user + "', '" + n.pass + "', " + n.question1 + ", '" + n.answer1 + "', " + n.question2 + ", '" + n.answer2 + "', " + n.question3 + ", '" + n.answer3 + "', " + position +");");
        ...
        public static bool GetSales(string start, string end)
        {
            try
            {
                dataAdapter = new SqlDataAdapter();
                dataTable = new DataTable();
                command = new SqlCommand("Select o.OrderID, OrderDate, o.PersonID, (p.NameFirst + ' ' + p.NameLast) as [Name], ItemName, d.Quantity, (d.Quantity * RetailPrice) as [Total] From " + SCHEMA_NAME + "Orders as o Join " + SCHEMA_NAME + "OrderDetails as d On o.OrderID = d.OrderID Join " + SCHEMA_NAME + "Inventory as i on d.InventoryID = i.InventoryID Join " + SCHEMA_NAME + "Person as p On o.PersonID = p.PersonID Where OrderDate Between '" + start + "' And '" + end + "' Order By OrderDate, OrderID;", connection);
                dataAdapter.SelectCommand = command;
                dataAdapter.Fill(dataTable);
                return true;
            }
